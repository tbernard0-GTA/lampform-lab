"""v0.8 section-aware beam-network surrogate; v0.6 solver is preserved.
...
def solve_design(part, tool, lattice, parameters=None):
    ...
    def cosines(xyz):
        xy=xyz[:,:2]
        v1 = xy[tri[:, 1]] - xy[tri[:, 0]]; v2 = xy[tri[:, 2]] - xy[tri[:, 0]]
    ...
    height_ratio = lattice['height'] / lattice['baseline_height']
    ...
    axial_weight = np.sqrt(width_ratio * height_ratio * rest / reference)
    bend_weight = np.sqrt(config['bending_weight'] *
                          np.mean(height_ratio[pair_edges] * width_ratio[pair_edges] ** 3, axis=1) *
    ...
    metrics.update(mean=float(parent_demand.mean()), maximum=float(parent_demand.max()), std=float(parent_demand.std()),
                   ...
                   critical_edges=int(np.sum(parent_demand > 30)), edge_count=len(parent_demand))
